...
"""
The Following are Functions and Classes necessary to compute the dlscore
"""


#Calculation of dlscore
def calculate_score(lig_array, receptor, VINA_EXECUTABLE, nb_nets, temp_filename, rec, line_header, model_path):
    d = binana(lig_array, receptor, VINA_EXECUTABLE, line_header, temp_filename, rec)
# def calculate_score(lig_array, receptor, nb_nets, temp_filename, rec, line_header):
#     d = binana(lig_array, receptor, line_header, temp_filename, rec)

    with open(os.path.join(model_path, "pickles", "networks.pickle"), 'rb') as f:
        nets = pickle.load(f)
        f.close()
...
#             print (line_header + "The output of network #" + str(len(scores) + 1) + " could not be determined because of an overflow error!")
    ...
    with open(os.path.join(model_path, "pickles/transform.pickle"), "rb") as f:
    ...
    count = 1
    #nb_nets = 10
    for dl_net in dl_nets(nb_nets, model_path):
    ...
    res = {}
    res['dlscore'] = dlscores

    return res

#Necessary for Binana Class
class MathFunctions:
...
#Necessary to manipulate input data
class binana:
    ...
    def __init__(self, ligand_pdbqt_filename, receptor_pdbqt_filename, vina_exec, line_header, actual_filename_if_ligand_is_list="", actual_filename_if_receptor_is_list=""): # must be a more elegant way of doing this
    #def __init__(self, ligand_pdbqt_filename, receptor, line_header, actual_filename_if_ligand_is_list="", actual_filename_if_receptor_is_list=""): # must be a more elegant way of doing this

        receptor = receptor_pdbqt_filename
        ...
        # Now get vina
        vina_output = getCommandOutput2(vina_exec + ' --score_only --receptor ' + receptor_pdbqt_filename + ' --ligand ' + ligand_pdbqt_filename)
...
#Used to iterate over multiple networks
def dl_nets(nb_nets, model_path):
    """ Yields feed forward nerual nets from the network directory """
    # Read the networks
    with open(os.path.join(model_path, "pickles", "sorted_models.pickle"), 'rb') as f:
        model_files = pickle.load(f)
    with open(os.path.join(model_path, "pickles", "sorted_weights.pickle"), 'rb') as f:
    ...
    for i, (model, weight) in enumerate(zip(model_files, weight_files)):
        if i==nb_nets:
            break
        # Load the network
        with open(os.path.join(model_path, "jsons", model), 'r') as json_file:
            loaded_model = model_from_json(json_file.read())

        # Load the weights
        loaded_model.load_weights(os.path.join(model_path, "weights", weight))
