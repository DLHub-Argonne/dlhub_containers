...
class Tree(object):
    ...
    def get(self, name=None):
        ...
        if name is None:
            ...
            if name in self.nodes:
                ...
                raise KeyError('No node named %s' % str(name))
    ...
    def add_child(self, name, node):
        ...
        if name in self.nodes.keys():
            raise KeyError('name, %s, already in use in tree' % str(name))
        else:
            node.name = name
    ...
    def rename_node(self, old_name, new_name):
        ...
        if not self.nodes.has_key(old_name):
            ...
            raise KeyError('No node named %s' % str(old_name))
        if self.nodes.has_key(new_name):
            raise KeyError('There already is a node name %s' % str(new_name))
        else:
