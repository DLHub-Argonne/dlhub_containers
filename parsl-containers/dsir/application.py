...
def run(data):
    ...
    all_images = []
    # Account for old and new file extension of tiff files
    tif_files = glob.glob(join(file_path, "*.tif"))
    tiff_files = glob.glob(join(file_path, "*.tiff"))
    all_images.extend(tif_files)
    all_images.extend(tiff_files)
    assert len(all_images) > 1 #Ensure there are images to run model on
...
def decode_img(img_zoom):
    img_zoom_scale = resize(img_zoom, (red_sqr*8, red_sqr*8), preserve_range=True, mode='constant', anti_aliasing=True)
